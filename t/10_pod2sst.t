...
# An even more intensive test of the parser.

# This test reads on the Pod at the end of the file, then compares it
# with the SST prologue in the __DATA__ section.
...
# it in a scalar, until we hit the =pod.
my @reference;
while( my $line = <DATA> ) {
  last if $line =~ /^=pod/;
  chomp $line;
...
isa_ok( $parser->prologue, "Starlink::Prologue" );

# Stringify the parsed data, then separate by carriage returns.
my $string = $parser->prologue->stringify;
my @parsed = split /\n/, $string;

is_deeply( \@parsed, \@reference );
...
#     subtraction, flat-field creation and division, feature detection
#     and matching between object frames, and resampling. See the
...
#     to give the most consistent result amongst the overlapping
#     regions. The mosaic is not trimmed to the dimensions of a single
#     frame, thus the noise will be greater in the peripheral areas
#     having received less exposure time. The mosaic is not normalised
#     by its exposure time (that being the exposure time of a single
#     frame).
...
#     the end airmass and end UT headers are updated to match that of
#     the last-observed frame contributing to the mosaic.
#     -  Intermediate frames are deleted except for the flat-fielded
#     (_ff suffix) frames.

#  References:
#     "Scripts for UFTI" G.S. Wright & S.K. Leggett, 1997 orac009-ufts,
#     v01.
...
#        number of offsets, as given by FITS header NOFFSETS, minus one
...
#  Output Data:
#     -  The integrated mosaic in <m><date>_<group_number>_mos, where
...
#  Related Recipes:
#     JITTER_SELF_FLAT_APHOT, JITTER_SELF_FLAT_BASIC,
#     JITTER_SELF_FLAT_NO_MASK, JITTER_SELF_FLAT_TELE,
#     MOVING_JITTER_SELF_FLAT_TELE, QUADRANT_JITTER.
